...
namespace WiiDownloader
{
    public partial class WiiDownloader_Form : Form
    {
        ...
        string WiiDownloader_version = "5.0";
        ...
        public WiiDownloader_Form()
        {
            ...
            FileDelete(CombinePath(STARTUP_PATH, "Database changelog.txt"));            
                ...
                foreach (Process clsProcess in Process.GetProcesses())
                {
                    ...
                    if (process_checked == processToCheck)
                    {
                        ...
                        if (loop_count < 30)
                            ...
                            return false; // dopo 30 secondi che nusd non ha creato il WAD (a download compleato) c'è qualcosa che non va.
                    }
                }
            }
            else
        ...
        private bool ToolsFileCheck(string path, string fileToCheck)
        {
            ...
            if (md5_from_file != md5)
            {
                ...
                MessageBox.Show("MD5 check failed for '" + fileToCheck + "'", "WiiDownloader (Error)", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return true;
        }
        ...
        private void createDictionary()
        {
            ...
            checkBoxDownloadNeogamma.Text = Dictionary.Download + " Neogamma";
            ...
            if (dictionary.IniReadValue("message", "infoForFirewall") != "")
                Dictionary.InfoForFirewall = dictionary.IniReadValue("message", "infoForFirewall");
            else
                Dictionary.InfoForFirewall = "WiiDownloader seem to have problem using external applications: try do add some exception to your firewall/antivirus, or disable them temporally..";           
       ...
       // ButtonValueForStopping
        ...
        public bool executeDownload(string Text4Label, string url, string file_name, string Text4LabelError)
        {
            ...
            MAX_DELAY_FOR_TYMER = 100000; // 100 secondi di inattività prima del timeout
            ...
            {
                string err_msg = "\n" + Dictionary.ErrorOccurred + " '" + Path.GetFileName(file_name) + "' NOT valid. FileSize=ZERO (error #30)\n";
                nextTimeWait = true;
                MySleep(500);
                AppendText(err_msg);                
        ...
        {          
            ...
            if (program.Contains("EditedModMii.bat")) 
                ...
                MAX_DELAY_FOR_TYMER = 300000; // 300 secondi (5 minuti) per gli altri programmi bastano (a meno che non si scompatti uno zip da 5 GB..=P )
            ...
            if (timeOut == false && DOWNLOAD_OR_PROGRAM_WORKING == true)
            {
                ...
                {
                    stopUsedProcess(short_program);
                    AppendText(".." + Dictionary.ErrorOccurred + " unexpcted error (error #50) using '" + short_program + "'\n");
                    return false;
                }
            }
            ...
            if (timeOut == true || DOWNLOAD_OR_PROGRAM_WORKING == false)
            {
                if (timeOut == true)
                {
                    AppendText("\n.." + Dictionary.ErrorOccurred + " " + "Time out." + Text4LabelError + " (error #21)" + "\n");
                    MessageBox.Show(Dictionary.InfoForFirewall, "WiiDownloader - Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private bool ExecuteCommandForNusd(string Text4Label, string path_for_wad, string ID, string version, string title_for_Wad, string Text4LabelError)
        {
            ...
            MAX_DELAY_FOR_TYMER = 60000; // 1 minuto di inattività prima del timeout
            TYMER_INTERVAL = 100; // controllo i dati scaricati ogni 10 centesimi di secondo                              
            ...
            catch (Exception ex)
            {
                ...
                {
                    AppendText("\n" + Dictionary.ErrorOccurred + " " + ex.Message + Text4LabelError + " (error #13)" + "\n");
            ...
            {                
                ...
                {
                    stopUsedProcess("nusd");
                    AppendText(".." + Dictionary.ErrorOccurred + " unexpected error (error #51) using 'nusd'\n");
                    return false;
                }
            }
            ...
            if (timeOut == true)
            {
                ...
                AppendText(Dictionary.ErrorOccurred + " " + "Time out. " + Text4LabelError + " (error #22)" + "\n");
            ...
            foreach (string file in System.IO.Directory.GetFiles(CombinePath(TOOLS_PATH, ID)))
            {
                ...
                {                  
                    ...
                    {
                        AppendText(Dictionary.ErrorOccurred + " " + "MD5 check failed. " + Text4LabelError + " (error #31)" + "\n");
                        return false;
                    }
        ...
        private bool Download_Modmii_base(string cios_version, string cios_base)
        {
            ...
            if (!NetworkOk)
            {
                AppendText("\n" + Dictionary.error + " " + Dictionary.Downloading + " " + name_for_text_box + " : " + Dictionary.errorDownloadingWithNoNetwork + " (error #01)" + "\n");
                DOWNLOAD_OR_PROGRAM_WORKING = false;
                return false;
            }
            ...
            {
                AppendText("\n" + Dictionary.error + " " + Dictionary.Downloading + " " + wad_name + " : " + Dictionary.errorDownloadingWithNoNetwork + " (error #01)" + "\n");
                DOWNLOAD_OR_PROGRAM_WORKING = false;
                return false;
            }
                    ...
                    if (!LaunchModMiiBatch(wad_name))
                        ...
                        else
                        {
                            ...
                            if (cios_version == "System menu patched IOS")
                            {
                                ...
                                if (!OtherMD5Check(localOutputFile, wad_name, localOutputFile))
                                {
                                    if (RETRY_TO_DO > 0)
                                    {
                                        RETRY_TO_DO--;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!OtherMD5Check(outputFile, wad_name, localOutputFile))
                                    {
                                        if (RETRY_TO_DO > 0)
                                        {
                                            RETRY_TO_DO--;
        ...
        private bool OtherMD5Check(string filepath, string wad_name, string filePath)
        {
            bool wadOk = true; 
            string localOutputFileMD5 = GetMD5HashFromFile(filePath);

            switch (wad_name)
            {
                case "cIOS247[37]-d2x-v8-final.wad":
                    if ("2ebee5deb62d944a1a03fea1aff2b16b" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS248[38]-d2x-v8-final.wad":
                    if ("7c283a1cedc3127019e32be945d97fe5" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS249[56]-d2x-v8-final.wad":
                    if ("15b4f42b7c174bc59dde005d6308174b" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS250[57]-d2x-v8-final.wad":
                    if ("7f3d117827fcd955f5675d895ae2a962" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS251[58]-d2x-v8-final.wad":
                    if ("94f50b592804e1f6103c0cdd1d106d92" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS247[37]-d2x-v10-beta52.wad":
                    if ("3e31df3e06b5155dc93ccf564079069e" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS248[38]-d2x-v10-beta52.wad":
                    if ("fd21adc208f66af552e7bc64de77a447" != localOutputFileMD5) 
                        wadOk = false;
                    break;
                case "cIOS249[56]-d2x-v10-beta52.wad":
                    if ("456c2d9c152a9695763400beb8a8ae05" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS250[57]-d2x-v10-beta52.wad":
                    if ("da3f51e01f511d6c8e7006b8cc294620" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS251[58]-d2x-v10-beta52.wad":
                    if ("dadc2ec9662b238702397dc022465dc5" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS247[37]-d2x-v10-beta53-alt.wad":
                    if ("fdbb2016b7ef7f700f045b5109ac4b9d" != localOutputFileMD5)                         
                        wadOk = false;
                    break;
                case "cIOS248[38]-d2x-v10-beta53-alt.wad":
                    if ("0b092a93fcc16a88de5786f39e27192c" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS249[56]-d2x-v10-beta53-alt.wad":
                    if ("021871027856fbc085a0dcc604e6bcb3" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS250[57]-d2x-v10-beta53-alt.wad":
                    if ("07f297d47b0b0482854dafa66b569436" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS251[58]-d2x-v10-beta53-alt.wad":
                    if ("17838e4f2354fd1cd98b0502d5975355" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS202[60]-HermesRodries-v5.1.wad":
                    if ("7e455ddeeac4f831f9235b8d37a36078" != localOutputFileMD5)
                        wadOk = false;
                    break;               
                case "cIOS222[38]-Hermes-v4.wad":
                    if ("ea7e5086139c2db3d27e8a64591682d1" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS222[38]-Hermes-v5.wad":
                    if ("77f1df39a24d312f988cecf4dd68e582" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS222[38]-HermesRodries-v5.1.wad":
                    if ("99404fcf2af266469d93fd2ae8f0fe0a" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS223[37-38]-Hermes-v4.wad":
                    if ("155ba8c52cdf154af77cf31dcc61a56f" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS223[37]-Hermes-v5.wad":
                    if ("a2cf208d51cea80b82059937778c15b7" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS223[37]-HermesRodries-v5.1.wad":
                    if ("488f09d029346edd84701f789c99bd31" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "cIOS224[57]-Hermes-v5.wad":
                    if ("ce67b890fd6dd804d75ae18093fc4235" != localOutputFileMD5)
                        wadOk = false;
                    break;                
                case "cIOS224[57]-HermesRodries-v5.1.wad":
                    if ("f9d9b2967b4568e7d3c304c5e43d4952" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS11v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("32f25aaefd0bb4cf53b76c2e984d12f7" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS20v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("808f1d1f073adc9dc0406ead1d43a318" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS30v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("a68689125e3172faa04c00ef42a593b1" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS40v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("dd8fe0f58a019f45c792b92837125cd8" != localOutputFileMD5)  
                        wadOk = false;
                    break;
                case "IOS50v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("3b1f2a76fd800ddb777744a47086844b" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS52v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("62fb16adb16b6b3823ef99746caaef03" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS60v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("c45a2bf6d560b2039f900f57cb00e7e1" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS70v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("2ddf180d37b35f1e9abb6982fe55f587" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS80v16174(IOS60v6174[FS-ES-NP-VP-DIP]).wad":
                    if ("23f3008dca379b5db2044bf09ec03276" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS11v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("9391e0e0edd627937ca4f5ceaf4358e4" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS20v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("3bd09e98a48600f367c11e52b01ce258" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS30v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("f3066f78fcfab808cdf275b4d5ff1039" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS40v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("d7ae1b0c6620b39100ccd631ac1d8ae7" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS50v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("d9595512d9b93ed04a537cf14e13755c" != localOutputFileMD5)  
                        wadOk = false;
                    break;
                case "IOS52v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("056b6a25fbb22a854ea996714b17813e" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS60v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("00a6471bc0a4ead6d44a5523e376a9c4" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS70v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("34d3ef5d4ae165e6149779bf13a3e59c" != localOutputFileMD5)
                        wadOk = false;
                    break;
                case "IOS80v16174(IOS60-v6174[FS-ES-NP-VP]).wad":
                    if ("a6c106fe14f58a155b05de9334ea4d62" != localOutputFileMD5)  
                        wadOk = false;
                    break; 
                default:
                   // questo per mettere gli MD5 su file per facilitarmi le cose
                  /*using (StreamWriter sw = File.AppendText(CombinePath(STARTUP_PATH, "md5.txt")))
                    {
                        sw.WriteLine(wad_name + " " + localOutputFileMD5);  
                        sw.WriteLine("\r\n");
                    }*/
                    wadOk = true; // I HOPE !!! ^__^ .. non posso controllarli tutti...
                    break;
            }

            if (!wadOk)
            {
                AppendText("\n" + Dictionary.ErrorOccurred + " " + "MD5 check failed for " + wad_name + " (error #33)\n");
                FileDelete(filepath); 
                return false;
            }            
                ...
                if ((title != null) && (title.Trim() != ""))
                {
                    ...
                    {                        
                        ...
                        while (continueSingleNusDownload == true)
                        {
                            ...
                            if (!NetworkOk)
                            {
                                AppendText("\n" + Dictionary.error + " " + Dictionary.Downloading + " " + titleToDownload + " : " + Dictionary.errorDownloadingWithNoNetwork + " (error #01)" + "\n");
                                DOWNLOAD_OR_PROGRAM_WORKING = false;
                                return false;
                            }
                            ...
                            else
                            {
                                ...
                                {
                                    AppendText("\n" + Dictionary.error + " " + Dictionary.Downloading + " " + titleToDownload + " (error #52)" + "...\n");
                                    DialogResult myDialogResult;
                                ...
                                // aggiungo la base scaricata alla cache, con alcune eccezioni.. è solo in caso di IOS da pacthare
                                if (!skip_basewad_download && !titleToDownload.Contains("-64-v") && titleToDownload.Contains("IOS"))                             
                                    addWadToCache(fileDownloaded, titleToDownload.Replace(@"v", @"-64-v"), ID + "v" + version);
                                ...
                                if (patch != "" || newSlot != "" || newVersion != "")
                                {
                                    ...
                                    if (filePatched.Trim() == "")
                                    {
                                        AppendText("\n" + Dictionary.error + " " + Dictionary.Patching + " " + originalIOS + " (error #53)" + "...\n");
                                        errorCount++;
                                    ...
                                    {
                                        if (!OtherMD5Check(CombinePath(dest, filename), filename, filePatched))
                                            return false;                                        
                ...
                if (d.IsReady)
                {
                    switch (d.DriveType)
                    {
                        ...
                        case DriveType.Removable:
                            if (STARTUP_PATH.Substring(0, 1).ToUpper() == d.Name.Substring(0, 1).ToUpper())
                            {
                                MessageBox.Show("WiiDownloader can't be started from a removable drive:\nApplication will be close.\n\nRun it form your local drive.\n(from a folder in your desktop is a good idea...)", this.Text + " - " + Dictionary.error, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return false;
                            }
                            break;
                        default:
        ...
        private bool moveFileToRoot(string scriptName, string fileToTheRoot)
        {
            ...
            {
                AppendText(" " + Dictionary.error + ": " + Dictionary.FileNotFound + " " + fileToTheRoot + " (error #02)");
                return false;
            }
        ...
        private bool CountDownloadToDo(string group, string scriptType, string scriptName)
        {
            ...
            {
                AppendText("\n" + Dictionary.error + ": " + Dictionary.InvalidScript + "\n" + '"' + group + "\" > \"" + scriptType + "\" > \"" + scriptName + "\" not found." + " (error #60)");
                return false;
            }
        ...
        private bool EditingModMiiBatch()
        {
            ...
            foreach (string line in file)
            {
                ...
                else if(line.Contains("ModMii_Log.bat (call temp"))
                {
                   // temp = line.Replace("if exist temp\\ModMii_Log.bat (call temp\\ModMii_Log.bat)", "goto:miniskip");
                    newFile.Append("goto:miniskip\r\n");
                    lineFound = true;
                    continue;
                }
        ...
        private bool checkForFreeSpace(string source, string destination, bool is_only_a_file)
        {
            ...
            {
                NoFreeSpace = true;
                AppendText("\n" + Dictionary.ErrorOccurred + " " + Dictionary.NoFreeSpace1 + destination_for_file + Dictionary.NoFreeSpace2 + " (error #70)" + "\n");
        ...
        private bool StandardDownload(string group, string scriptType, string scriptName)
        {
            ...
            while (continueDownload == true)
            {
                ...
                if (urlType == "dinamic") 
                {
                    ...
                    if (linkToProcess == "")
                    {
                        AppendText("\n" + Dictionary.error + ": " + Dictionary.NoResponse + " " + link + " (error #23)" + "\n");
                        goto Error;
                    }
                    AppendText("...OK!\n");
                }
                ...
                {
                    AppendText(Dictionary.ErrorOccurred + " " + "MD5 check failed for " + downloadedFile + " (error #32)");                    
                    goto Error;
                }
        ...
        private void LoadWiiInfo(bool onlyForCreateGuide)
        {
            ...
            if (ini.IniReadValue("WiiInfo", "checkBoxDownloadNeogamma") == "True")
                checkBoxDownloadNeogamma.Checked = true;
            else if (ini.IniReadValue("WiiInfo", "checkBoxDownloadNeogamma") == "False")
                checkBoxDownloadNeogamma.Checked = false;            
        ...
        private bool WiiInfoSave()
        {
            ...
            ini.IniWriteValue("WiiInfo", "checkBoxDownloadNeogamma", Convert.ToString(checkBoxDownloadNeogamma.Checked));            
        ...
        {            
            ...
            {
                if (comboBoxCIOSes.SelectedIndex == 0)
                {
                    FileDelete(CombinePath(TEMP_SCRIPT_PATH, "temp_script", "Recommended cIOSs"));
                    FileMove(CombinePath(STANDARD_SCRIPT_PATH, "WAD - cIOS", "Recommended cIOSs [beta52+hermes]"), CombinePath(TEMP_SCRIPT_PATH, "temp_script", "Recommended cIOSs"), false);
                ...
                {
                    FileDelete(CombinePath(TEMP_SCRIPT_PATH, "temp_script", "Recommended cIOSs"));
                    FileMove(CombinePath(STANDARD_SCRIPT_PATH, "WAD - cIOS", "Recommended cIOSs [beta53+hermes]"), CombinePath(TEMP_SCRIPT_PATH, "temp_script", "Recommended cIOSs"), false);
                    IniFile scriptFile = new IniFile(CombinePath(TEMP_SCRIPT_PATH, "temp_script", "Recommended cIOSs"));
                    scriptFile.IniWriteValue("info", "copyCiosTo", CombinePath("WAD", "WAD for Modding"));
                    lines = lines + "temp, temp_script, Recommended cIOSs;";
                }     
            ...
            if ((checkBoxDownloadNeogamma.Checked == true) || (checkBoxDownloadCFG.Checked == true) || (checkBoxDownloadGX.Checked == true) || (checkBoxDownloadWiiFlow.Checked == true))
            {
                if (checkBoxDownloadNeogamma.Checked == true)
                {
                    FileDelete(CombinePath(TEMP_SCRIPT_PATH, "temp_script", "NeoGamma R9 - Beta56 [autoboot full channel]"));
                    FileMove(CombinePath(STANDARD_SCRIPT_PATH, "Homebrew - Loader - Neogamma", "NeoGamma R9 - Beta56 [autoboot full channel]"), CombinePath(TEMP_SCRIPT_PATH, "temp_script", "NeoGamma R9 - Beta56 [autoboot full channel]"), false);
                    IniFile channelFile = new IniFile(CombinePath(TEMP_SCRIPT_PATH, "temp_script", "NeoGamma R9 - Beta56 [autoboot full channel]"));
                    channelFile.IniWriteValue("info", "copyTo", CombinePath("WAD", "WAD for Modding"));
                    lines = lines + "temp, temp_script, NeoGamma R9 - Beta56 [autoboot full channel];";
                }
                if (checkBoxDownloadCFG.Checked == true)
                {
                    // prima era così, ora dalla v 3.2 ho cambiato nel caso escano nuove relase complete e finali (e NON siano necessari altri script)
                ...
                if (checkBoxDownloadWiiFlow.Checked == true)
                {
                  ...
                  // languages pack is already present in "setting for new install"  
        ...
        private void checkBoxCreateScript_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (checkBoxCreateScript.Checked == true)
            {
                ...
                comboBoxCIOSes.SelectedIndex = 0;
                if(dictionary.IniReadValue("menu", "saveAndCreateScript") != "")
                    buttonSaveWiiInfo.Text = dictionary.IniReadValue("menu", "saveAndCreateScript");                
                else
                    buttonSaveWiiInfo.Text = "Save and create script";
        ...
        {            
            ...
            if (File.Exists(tutorialFile))
            {
                ...
                if (    (checkBoxDownload41.Checked) ||
                        ...
                        (checkBoxDownloadNeogamma.Checked) ||
        ...
        private void WAM_check()
        {
            ...
            if (checkBoxDownloadCIOS.Checked)
                comboBoxCIOSes.Enabled = true;
            else
                comboBoxCIOSes.Enabled = false;
        }
...
//            WAM_check();
        ...
        private void checkBoxDownloadNeogamma_CheckedChanged(object sender, EventArgs e)
        {
            WAM_check();
        }                      
    }
